...
describe('iPhone 14 Pro Max 首页通知条 / 纳税明细字号', () => {
  ...
  it('年度选择页首屏打 14promax，并放大标题/年度行', () => {
    ...
    expect(shuiming).toContain('auth.js?v=20260918-ios27-edge');
  });
  ...
  it('我的页 14 Pro Max 单独回退到 9/1 底图，隐藏 HTML 胶囊避免叠字', () => {
    ...
    expect(mine).toContain('auth.js?v=20260918-ios27-edge');
  ...
  it('登录页 14 Pro Max 放大标题/输入并加宽表单', () => {
    ...
    expect(login).toContain('auth.js?v=20260918-ios27-edge');
  });
});
