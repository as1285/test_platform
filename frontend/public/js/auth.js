...
(function () {
  ...
  /**
   * vivo S15（国行 V2203A）。OriginOS 4 Cordova 常忽略 overlays=false，
   * 又被 vivo 族当成「外置栏 → inset 0」，首页/我的/办查会把蓝白铺进状态栏，
   * 正版个税则是顶部黑框。页内铺 40px 黑垫，勿匹配 S15 Pro / S15e。
   */
  function isVivoS15Client() {
    var ua = clientUaBlob();
    if (/V2203A|V2203T|PD2203\b/i.test(ua)) {
      return true;
    }
    if (/S15\s*Pro|S15e|S15[\s_-]*e/i.test(ua)) {
      return false;
    }
    return /(?:vivo[\s_-]*)?S15\b/i.test(ua);
  }
  ...
  /**
   * iQOO Z9 Turbo+（国行 V2417A / 产品码 PD2417）：OriginOS Cordova WebView 360×800。
   * 「我的」与 X90 同一坑：@sm 双层底图会叠出半透明白卡，100% 100% 再把 1242rpx 压进 1180。
   * 只进单层底图档，勿把 Z9 Turbo / Z9 / Z9x 算进来。
   */
  function isIqooZ9TurboPlusClient() {
    var ua = clientUaBlob();
    if (/V2417A|V2417DA|PD2417\b/i.test(ua)) {
      return true;
    }
    return /(?:iQOO|iqoo)?[\s_-]*Z9[\s_-]*Turbo[\s_-]*(?:\+|Plus)/i.test(ua);
  }
  ...
  /**
   * Galaxy S23 Ultra（国行 SM-S9180，385×824@3.75）：打孔状态栏盖在 WebView 上。
   * 不能跟其它三星 One UI 一样把顶距清零，否则收入纳税明细标题会顶进时间栏。
   * 不含 S23 / S23+ / S24 Ultra。
   */
  function isSamsungS23UltraClient() {
    var ua = clientUaBlob();
    return /SM-S918|Galaxy\s*S23\s*Ultra|S23[\s_-]*Ultra/i.test(ua);
  }
  ...
  function isAndroidOuterStatusBarClient() {
    ...
    return (
      ...
      isRedmiK70Client() ||
      (isSamsungOneUiFamilyClient() && !isSamsungS23UltraClient()) ||
      isHuaweiHarmonyOsFamilyClient()
    );
  }
  ...
  /**
   * 小米 13 Ultra（2304FPN6DC / 2304FPN6DG）：6.73" 3200×1440。
   * 勿并入 13 / 13 Pro；HyperOS Cordova 仍压白状态栏，收入纳税明细须留 40px。
   */
  function isXiaomi13UltraClient() {
    var ua = clientUaBlob();
    if (/2304FPN6/i.test(ua)) {
      return true;
    }
    return /(?:Xiaomi|Mi|小米)[\s_-]*13[\s_-]*Ultra/i.test(ua);
  }

  /** 小米 13（2211133C 等，不含 13 Pro / 13 Ultra）。HyperOS Cordova 仍压白状态栏，收入纳税明细须留 40px。 */
  function isXiaomi13Client() {
    var ua = clientUaBlob();
    if (isXiaomi13ProClient() || isXiaomi13UltraClient()) {
      return false;
    }
    return /2211133[CGI]|(?:Xiaomi|Mi|小米)[\s_-]*13\b(?![\s_-]*(?:Pro|Ultra|Lite))/i.test(ua);
  }
  ...
  /** 红米 K70 / K70E 标准版：只认型号 / 营销名，不用分辨率兜底。 */
  function isRedmiK70StandardClient() {
    return isRedmiK70StandardModelBlob(clientUaBlob());
  }

  /** 桌面设备模式 / AVD 预览 K70「我的」黑垫。不认真机 Pixel，不认其它 OEM。 */
  function isMineUnderlapPreviewBlob(ua) {
    ...
    /* Chrome 设备模式会把 UA 改成 Pixel/Android，platform 仍是 Win32 / MacIntel */
    try {
  ...
  function isMineStatusPage() {
    ...
    try {
      return /mine\.html$/i.test(String(location.pathname || ''));
    } catch (ePath) {
      return false;
    }
  }
  ...
  function resolveMineStatusMode() {
    if (typeof window.resolveMineStatusMode === 'function' && window.resolveMineStatusMode !== resolveMineStatusMode) {
      try {
        var bootMode = window.resolveMineStatusMode();
        if (bootMode === 'underlap-black') {
          return bootMode;
        }
      } catch (eBoot) {}
    }
  ...
  function shouldApplyMineBlackStatus() {
    return resolveMineStatusMode() === 'underlap-black';
  }
  try {
  ...
  function isIPhone15PlusProMaxLikeClient() {
    ...
    try {
      if (document.documentElement.classList.contains('app-ios-iphone14promax')) {
        return false;
      }
    } catch (e14) {}
    var ua = clientUaBlob();
  ...
  /**
   * iPhone 14 Pro Max（iPhone15,3，430×932 灵动岛）。
   * Safari 常无型号；同逻辑屏且无 15 系列 UA 时走此档，底栏保持悬浮胶囊。
   */
  function isIPhone14ProMaxClient() {
    if (!isLikelyIOSViewportClient()) {
      return false;
    }
    var ua = clientUaBlob();
    ...
    if (isIPhone16ProMaxClient() || isIPhone17ProMaxClient()) {
      return false;
    }
    var sides = getIOSLogicalScreenSides();
    if (!sides) {
  ...
  function isIPhone14ProMaxCapsuleNavClient() {
    ...
    return isIPhone14ProMaxClient();
  }

  function isIPhone15ProMaxDockNavClient() {
    try {
      if (isIPhone14ProMaxCapsuleNavClient()) {
        return false;
      }
    } catch (e14) {}
    try {
  ...
  function pinMineE1RpxFromCanvas() {
    try {
      ...
      var note13pro =
        isRedmiNote13ProClient() || root.classList.contains('app-android-redmi-note13-pro');
      var matepad115s =
      ...
      if (mate60 || mi14pro || p40pro || acepro || reno10 || hinova9se || xiaomi13ultra || note13pro || matepad115s) {
  ...
  function isHyperOs2MineE1SmClient() {
    ...
    if (
      isRedmiNote13ProClient() ||
      document.documentElement.classList.contains('app-android-redmi-note13-pro')
    ) {
      return false;
    }
    if (
  ...
  function hyperOs2MineE1SmRootHit(root) {
    ...
    if (
      isRedmiNote13ProClient() ||
      root.classList.contains('app-android-redmi-note13-pro')
    ) {
      return false;
    }
    if (
  ...
  function xiaomi14ProMineE1LockCss() {
    ...
    HYPEROS2_MINE_E1_SM_CLASSES.forEach(function (cls) {
      ...
      if (cls === 'app-android-mine-e1-sm') {
        rootSel +=
          ':not(.app-android-mine-e1-plainimg):not(.app-android-iqoo-13):not(.app-android-iqoo-15):not(.app-android-oneplus-acepro):not(.app-mine-black-status):not(.app-android-redmi-k70):not(.app-android-xiaomi-13ultra):not(.app-android-redmi-note13-pro):not(.app-android-oppo-reno10):not(.app-android-huawei-matepad115s)';
      }
      css +=
  ...
  function pinXiaomi13UltraMineE1Layout() {
    try {
      ...
      root.classList.add('app-android-xiaomi-13ultra');
      root.classList.add('app-android-client');
      root.classList.add('app-android-immersive-white-top');
      try {
        var oldLock = document.querySelector('style[data-xiaomi13ultra-mine-e1-lock]');
        if (oldLock && oldLock.parentNode) oldLock.parentNode.removeChild(oldLock);
        var lock = document.createElement('style');
  ...
  /**
   * 「我的」e1 单层底图档（vivo X90 / iQOO Z9 Turbo+ / OriginOS 5）。
   *
  ...
  function isMineE1PlainImgClient() {
    ...
    /* Z9 Turbo+ 只在「我的」走单层，避免 pin 把其它页也改成沉浸白顶 */
    if (isIqooZ9TurboPlusClient()) {
      try {
  ...
  function paintXiaomi14ProMineE1(src) {
    try {
      ...
      if (
        ...
        window.__mineE1ForceSm = true;
        var canvasN13 = document.getElementById('mineE1Canvas');
        var imgN13 = document.getElementById('headerImg');
        var urlN13 = mineE1ToSmUrl(src || (imgN13 && (imgN13.getAttribute('src') || imgN13.src)) || '');
        if (imgN13) {
          if (String(imgN13.getAttribute('src') || '') !== urlN13) {
            imgN13.src = urlN13;
          }
          imgN13.style.setProperty('opacity', '0', 'important');
          imgN13.style.setProperty('width', '1px', 'important');
          imgN13.style.setProperty('height', '1px', 'important');
        }
        if (canvasN13) {
          canvasN13.style.setProperty('background-image', 'url("' + urlN13 + '")', 'important');
          canvasN13.style.setProperty('background-size', '100% auto', 'important');
  ...
  function pinRedmiNote13ProMineE1Layout() {
    try {
      var root = document.documentElement;
      if (!isRedmiNote13ProClient() && !root.classList.contains('app-android-redmi-note13-pro')) {
        return;
      }
      root.classList.add('app-android-client');
      root.classList.add('app-android-redmi-note13-pro');
      ...
      if (document.body && document.body.classList.contains('page-mine')) {
        var tax = document.querySelector('body.page-mine .mine-ov-tax');
        if (tax) {
          tax.style.setProperty('top', 'calc(430 * var(--mine-rpx))', 'important');
        }
      }
      pinMineE1RpxFromCanvas();
      paintXiaomi14ProMineE1(img && (img.getAttribute('src') || img.src));
      if (!pinRedmiNote13ProMineE1Layout._rpxRearm) {
        pinRedmiNote13ProMineE1Layout._rpxRearm = true;
        [80, 240, 600, 1200].forEach(function (ms) {
          setTimeout(function () {
            try {
              pinRedmiNote13ProMineE1Layout();
            } catch (eN13) {}
          }, ms);
        });
      }
  ...
  function syncAppShellStatusbarTop() {
    try {
      ...
      if (isLikelyAndroidViewportClient()) {
        /*
         * 安卓黑状态栏靠页内 40px 固定黑条（HyperOS 常忽略 overlays=false）。
         * Ace2V 外置黑条顶距 0；Mate60 保留 52px 页内黑条。
         */
        if (isHuaweiMate60Client()) {
      ...
      if (measured < 20 && isIosStandaloneApp() && !isCordovaTaxAppShell()) {
        var islandBlueTopPage = false;
        try {
          islandBlueTopPage = !!(
            document.body &&
            (document.body.classList.contains('page-shouye') ||
              document.body.classList.contains('page-mine') ||
              document.body.classList.contains('page-daiban') ||
              document.body.classList.contains('page-bancha') ||
  ...
  function applyMinePageChrome() {
    try {
      ...
      try {
        if (resolveMineStatusMode() === 'underlap-black') {
      ...
      try {
        ...
        st.textContent =
          ...
          'html.app-top-safe-shell:not(.app-ios-iphone14promax) body.page-mine::before,' +
           ...
           * 一加 13 / MIX Fold / Mate 60 真沉浸另案；外置黑条机勿再垫 40px。
           */
          ...
          'html.app-android-redmi-k70.app-android-client.app-top-safe-shell:not(.app-android-redmi-k70-ultra) body.page-mine .mine-e1-canvas > #headerImg{margin-top:0 !important;}' +
          'html.app-mine-black-status body.page-mine .mine-e1-layer,' +
          'html.app-android-redmi-k70.app-android-client.app-top-safe-shell:not(.app-android-redmi-k70-ultra) body.page-mine .mine-e1-layer,' +
          'html.app-android-redmi-k70.app-android-client.app-cordova-shell:not(.app-android-redmi-k70-ultra) body.page-mine .mine-e1-layer{top:40px !important;}' +
    ...
    try {
      ...
      try {
        ...
        if (isXiaomi13ProClient() || syRoot.classList.contains('app-android-xiaomi-13pro')) {
          ...
          syRoot.classList.add('app-android-client');
        } else if (isXiaomi13UltraClient() || syRoot.classList.contains('app-android-xiaomi-13ultra')) {
          syRoot.classList.add('app-android-xiaomi-13ultra');
          syRoot.classList.add('app-android-client');
      ...
      pinIPhone14ProMaxZxkCard();
    } catch (e) {}
  }

  function pinIPhone14ProMaxZxkCard() {
    try {
      if (!document.body || !document.body.classList.contains('page-shouye')) {
        return;
      }
      ...
      var head = document.querySelector('.sy-zxk-card-head');
      if (head) {
        head.style.setProperty('align-items', 'center', 'important');
        head.style.setProperty('right', '0', 'important');
        head.style.setProperty('height', '17%', 'important');
      }
      var policy = document.querySelector('.sy-zxk-policy');
      if (policy) {
        policy.style.setProperty('color', '#ffffff', 'important');
        policy.style.setProperty('-webkit-text-fill-color', '#ffffff', 'important');
        policy.style.setProperty('font-weight', '700', 'important');
        policy.style.setProperty('font-size', '14px', 'important');
        policy.style.setProperty('align-self', 'stretch', 'important');
        policy.style.setProperty('justify-content', 'center', 'important');
        policy.style.setProperty('width', '27%', 'important');
        policy.style.setProperty('min-width', '0', 'important');
        policy.style.setProperty('padding', '0', 'important');
        policy.style.setProperty('height', 'auto', 'important');
      }
  ...
  try {
    window.pinIPhone14ProMaxZxkCard = pinIPhone14ProMaxZxkCard;
  } catch (eEx) {}
  ...
  /**
   * iOS 27 用 sticky 实底白采状态栏，fixed 顶栏不再自带 background。
   * Apple 在 iOS 27 忽略 UIDesignRequiresCompatibility，页内必须自己铺白。
   * 顶栏底那条灰线：isolation + 灰 box-shadow + 系统玻璃描边叠出来的，白垫要盖过底边。
   */
  function ios27StickyChromeCss() {
    return (
      ...
      'html.app-ios-sticky-chrome .ios-sticky-tint,#iosStickyTint.ios-sticky-tint{' +
      'position:sticky !important;top:0 !important;left:0 !important;right:0 !important;display:block !important;' +
      ...
      /* 描述文件 WebClip：fixed 灰 shield 会被 iOS 26/27 采成顶部奶白毛玻璃，必须关掉 */
      ...
      'html.app-ios-unified-chrome body.page-shuiming-result .top-fixed .summary,html.app-ios-liquid-glass body.page-shuiming-result .top-fixed .summary,html.app-ios-iphone15.app-top-safe-shell.app-ios-unified-chrome body.page-shuiming-result .top-fixed .summary{position:relative !important;top:auto !important;left:auto !important;right:auto !important;transform:none !important;-webkit-transform:none !important;background:#f5f6fa !important;padding:12px 0 10px !important;}' +
      'html.app-ios-unified-chrome body.page-shuiming-result .list,html.app-ios-liquid-glass body.page-shuiming-result .list,html.app-ios-unified-chrome.app-ios-iphone15.app-top-safe-shell body.page-shuiming-result .list{padding-top:0 !important;margin-top:0 !important;background:#f5f6fa !important;}' +
  ...
  function injectIos27StickyStyles() {
    try {
      var st = document.getElementById('ios27StickyChromeCss');
      if (!st) {
      ...
      var ov = document.getElementById('ios27InflowOverrideCss');
      if (!ov) {
  ...
  function isIosUnifiedFlowChrome() {
    try {
      ...
      /* 描述文件 WebClip / iOS 26+：必须走文档流 sticky 实白，否则 fixed shield 会被 Liquid Glass 采成奶白糊层 */
      if (root.classList.contains('app-ios-liquid-glass')) return true;
      return isIosLiquidGlassWebClip() || isIosLiquidGlassOS();
    } catch (e) {
      return false;
    }
  }
  ...
  function ensureIosStickyTintBar() {
    try {
      ...
      if (
        ...
        document.documentElement.classList.contains('app-ios-liquid-glass') ||
        isIosLiquidGlassWebClip() ||
        isIosLiquidGlassOS()
      ) {
        document.documentElement.classList.add('app-ios-unified-chrome');
      }
      injectIos27StickyStyles();
      /* 15 / iOS 26+ WebClip：tint 与 chrome-shield 是又一层 fixed，玻璃会重复采样奶白糊层，必须拿掉 */
  ...
  function nudgeIosLiquidGlassSample() {
    try {
      if (!isLikelyIOSViewportClient() || !isIosWhiteStatusPage()) return;
      var root = document.documentElement;
  ...
  function paintIosWhiteStatusRoot() {
    /* iOS 白顶页根上铺实底白 */
    try {
      if (!isIosWhiteStatusPage()) return;
      var root = document.documentElement;
    ...
    try {
      ...
      if (immersiveTopInsetClient) {
        try {
          ...
          if (isSamsungS23UltraClient() || root.classList.contains('app-android-samsung-s23u')) {
            root.classList.add('app-android-samsung-s23u');
            root.classList.add('app-android-samsung');
            root.classList.add('app-android-immersive-white-top');
            root.classList.remove('app-android-white-page-outer');
          }
          root.style.setProperty('--app-shell-statusbar-top', '40px');
  ...
  function ensureBottomNavLockStyle(opts) {
    ...
    var iosPad =
      ...
      '}' +
      'html.app-ios-client.app-ios-iphone14promax,html.app-ios-client.app-ios-iphone15promax,html.app-ios-client.app-ios-iphone13promax{--bottom-nav-side:0px!important;--bottom-nav-bottom:0px!important;--bottom-nav-gap:0px!important;--bottom-nav-radius:0px!important;--bottom-nav-clearance:calc(62px + env(safe-area-inset-bottom, 34px))!important;}' +
      'html.app-ios-client.app-ios-iphone14promax body.page-shouye,html.app-ios-client.app-ios-iphone14promax body.page-daiban,html.app-ios-client.app-ios-iphone14promax body.page-bancha,html.app-ios-client.app-ios-iphone14promax body.page-message,html.app-ios-client.app-ios-iphone14promax body.page-mine,' +
      ...
      'html.app-ios-client.app-ios-iphone14promax body.page-shouye > .bottom-nav,html.app-ios-client.app-ios-iphone14promax body.page-daiban > .bottom-nav,' +
      'html.app-ios-client.app-ios-iphone14promax body.page-bancha > .bottom-nav,html.app-ios-client.app-ios-iphone14promax body.page-message > .bottom-nav,' +
      'html.app-ios-client.app-ios-iphone14promax body.page-mine > .bottom-nav,' +
      'html.app-ios-client.app-ios-iphone15promax:not(.app-ios-iphone13promax) body > .bottom-nav,html.app-ios-client.app-ios-iphone15promax:not(.app-ios-iphone13promax) body > .bottom-nav.ios-device,' +
      'html.app-ios-client.app-ios-iphone15promax:not(.app-ios-iphone13promax) body.page-shouye > .bottom-nav,html.app-ios-client.app-ios-iphone15promax:not(.app-ios-iphone13promax) body.page-daiban > .bottom-nav,' +
      'html.app-ios-client.app-ios-iphone15promax:not(.app-ios-iphone13promax) body.page-bancha > .bottom-nav,html.app-ios-client.app-ios-iphone15promax:not(.app-ios-iphone13promax) body.page-message > .bottom-nav,' +
      ...
      '}' +
      'html.app-ios-client.app-ios-iphone14promax{--bottom-nav-side:16px!important;--bottom-nav-bottom:calc(env(safe-area-inset-bottom, 34px) + 8px)!important;--bottom-nav-gap:calc(env(safe-area-inset-bottom, 34px) + 8px)!important;--bottom-nav-height:54px!important;--bottom-nav-radius:32px!important;--bottom-nav-clearance:calc(54px + env(safe-area-inset-bottom, 34px) + 20px)!important;}' +
      'html.app-ios-client.app-ios-iphone14promax body > .bottom-nav,html.app-ios-client.app-ios-iphone14promax body > .bottom-nav.ios-device,' +
      'html.app-ios-client.app-ios-iphone14promax body.page-shouye > .bottom-nav,html.app-ios-client.app-ios-iphone14promax body.page-daiban > .bottom-nav,' +
      'html.app-ios-client.app-ios-iphone14promax body.page-bancha > .bottom-nav,html.app-ios-client.app-ios-iphone14promax body.page-message > .bottom-nav,' +
      'html.app-ios-client.app-ios-iphone14promax body.page-mine > .bottom-nav,html.app-ios-client.app-ios-iphone14promax body.page-mine > .bottom-nav.ios-device{' +
      'left:16px!important;right:16px!important;bottom:calc(env(safe-area-inset-bottom, 34px) + 8px)!important;width:auto!important;max-width:none!important;' +
      'border-radius:32px!important;height:54px!important;min-height:54px!important;max-height:54px!important;' +
      ...
      '}' +
      'html.app-ios-client.app-ios-iphone14promax body > .bottom-nav::before,html.app-ios-client.app-ios-iphone14promax body.page-shouye > .bottom-nav::before,' +
      'html.app-ios-client.app-ios-iphone14promax body.page-mine > .bottom-nav::before{content:none!important;display:none!important;}';
    var embedHide =
  ...
  function pinTabBottomNav() {
    ...
    var dockNav = false;
    var capsule14pm = false;
    try {
      capsule14pm = isIPhone14ProMaxCapsuleNavClient();
    } catch (e14c) {}
    try {
      dockNav = !capsule14pm && isIPhoneDockBottomNavClient();
    } catch (eDock) {}
    if (capsule14pm) {
      try {
        document.documentElement.classList.add('app-ios-iphone14promax');
        ...
        nav.style.setProperty('border-radius', '32px', 'important');
        ...
        document.documentElement.style.setProperty('--bottom-nav-side', '16px');
        document.documentElement.style.setProperty(
          '--bottom-nav-bottom',
        ...
        document.documentElement.style.setProperty('--bottom-nav-height', '54px');
        document.documentElement.style.setProperty('--bottom-nav-radius', '32px');
        document.documentElement.style.setProperty(
          '--bottom-nav-clearance',
          'calc(54px + env(safe-area-inset-bottom, 34px) + 20px)'
        );
      } catch (eCap) {}
      return;
    }
    if (dockNav) {
      try {
        ...
        if (document.documentElement.classList.contains('app-ios-iphone13promax')) {
          ...
          /* 14 / 15 Pro Max 正版：半透明贴底，模糊交给 CSS ::before，避免抬高 */
          nav.style.setProperty('box-shadow', '0 -0.33px 0 rgba(60,60,67,0.16)', 'important');
          nav.style.setProperty('background', 'rgba(255,255,255,0.52)', 'important');
        }
  ...
  function setupMobileStatusBar() {
    try {
      ...
      var xiaomi13UltraClient = androidClient && isXiaomi13UltraClient();
      ...
      var vivoS15Client = androidClient && isVivoS15Client();
      ...
      var iosIPhone14ProMax = iosClient && isIPhone14ProMaxClient();
      ...
      var samsungS23UltraClient = androidClient && isSamsungS23UltraClient();
        ...
        ? (xiaomiMixFoldClient
            ...
            : honorPgtAn20Client
              ...
              : androidOuterStatusBar ||
                  redmiK70Client ||
                  (samsungOneUiFamily && !samsungS23UltraClient) ||
                  huaweiHarmonyFamily ||
                  (xiaomiHyperOsFamily && !xiaomi14Client) ||
      ...
      if (xiaomi13UltraClient) {
        document.documentElement.classList.add('app-android-xiaomi-13ultra');
        document.documentElement.classList.add('app-android-immersive-white-top');
        document.documentElement.classList.remove('app-android-white-page-outer');
      ...
      if (vivoOriginOsFamily && !vivoImmersiveTop && !vivoS15Client) {
      ...
      if (samsungS23UltraClient) {
        document.documentElement.classList.add('app-android-client');
        document.documentElement.classList.add('app-android-samsung');
        document.documentElement.classList.add('app-android-samsung-s23u');
        document.documentElement.classList.add('app-android-immersive-white-top');
        document.documentElement.classList.remove('app-android-white-page-outer');
        try {
          document.documentElement.style.setProperty('--app-shell-statusbar-top', '40px');
      ...
      if (useTopSafeInset && statusInsetCss) {
        ...
        shellExtra.textContent =
          ...
          'html.app-ios-iphone16pro.app-top-safe-shell:not(.app-ios-status-outer),html.app-ios-iphone16promax.app-top-safe-shell:not(.app-ios-status-outer),html.app-ios-iphone15promax.app-top-safe-shell:not(.app-ios-status-outer),html.app-ios-iphone17promax.app-top-safe-shell:not(.app-ios-status-outer){--app-shell-statusbar-top:max(59px,env(safe-area-inset-top,59px)) !important;}' +
          'html.app-ios-status-outer.app-ios-iphone14promax,html.app-ios-status-outer.app-ios-iphone15promax{--app-shell-statusbar-top:0px !important;}' +
          'html.app-ios-status-outer.app-ios-iphone14promax body.page-shuiming-result .page-root,html.app-ios-status-outer.app-ios-iphone15promax body.page-shuiming-result .page-root,html.app-ios-status-outer.app-ios-iphone14promax.app-top-safe-shell body.page-shuiming-result .page-root,html.app-ios-status-outer.app-ios-iphone15promax.app-top-safe-shell body.page-shuiming-result .page-root{--safe-top:0px !important;--shuiming-chrome-top:0px !important;--app-shell-statusbar-top:0px !important;}' +
          'html.app-ios-status-outer.app-ios-iphone14promax body.page-shuiming-result .top-fixed .header,html.app-ios-status-outer.app-ios-iphone15promax body.page-shuiming-result .top-fixed .header,html.app-ios-status-outer.app-ios-iphone14promax.app-top-safe-shell body.page-shuiming-result .top-fixed .header,html.app-ios-status-outer.app-ios-iphone15promax.app-top-safe-shell body.page-shuiming-result .top-fixed .header{top:0 !important;height:var(--header-height,52px) !important;min-height:var(--header-height,52px) !important;padding:15px 16px !important;box-sizing:border-box !important;background:#fff !important;}' +
          'html.app-ios-status-outer.app-ios-iphone14promax body.page-shuiming-result .top-fixed .summary,html.app-ios-status-outer.app-ios-iphone15promax body.page-shuiming-result .top-fixed .summary,html.app-ios-status-outer.app-ios-iphone14promax.app-top-safe-shell body.page-shuiming-result .top-fixed .summary,html.app-ios-status-outer.app-ios-iphone15promax.app-top-safe-shell body.page-shuiming-result .top-fixed .summary{top:var(--header-height,52px) !important;}' +
          'html.app-ios-status-outer.app-ios-iphone14promax body.page-shuiming-result::before,html.app-ios-status-outer.app-ios-iphone15promax body.page-shuiming-result::before,html.app-ios-status-outer.app-ios-iphone14promax.app-top-safe-shell body.page-shuiming-result::before,html.app-ios-status-outer.app-ios-iphone15promax.app-top-safe-shell body.page-shuiming-result::before{content:none !important;display:none !important;height:0 !important;}' +
          'html.app-ios-status-outer.app-ios-iphone14promax body.page-shuiming > .header,html.app-ios-status-outer.app-ios-iphone15promax body.page-shuiming > .header,html.app-ios-status-outer.app-ios-iphone14promax.app-top-safe-shell body.page-shuiming > .header,html.app-ios-status-outer.app-ios-iphone15promax.app-top-safe-shell body.page-shuiming > .header{padding-top:14px !important;}' +
          'html.app-ios-status-outer.app-ios-iphone14promax body.page-shuiming > .content,html.app-ios-status-outer.app-ios-iphone15promax body.page-shuiming > .content,html.app-ios-status-outer.app-ios-iphone14promax.app-top-safe-shell body.page-shuiming > .content,html.app-ios-status-outer.app-ios-iphone15promax.app-top-safe-shell body.page-shuiming > .content{padding-top:46px !important;}' +
          /* iOS 我的：头图顶入，禁用拼接伪元素 */
          ...
          'html.app-ios-client.app-top-safe-shell body.page-mine .mine-fill-data-btn{top:calc(var(--mine-activate-btn-top-offset,66px) + var(--app-shell-statusbar-top,0px) + var(--mine-fill-data-gap,44px)) !important;left:16px !important;right:auto !important;}' +
          ...
          'html.app-ios-iphone12promax.app-top-safe-shell body.page-mine .mine-fill-data-btn{top:calc(var(--mine-activate-btn-top-offset,66px) + env(safe-area-inset-top,0px) + var(--mine-fill-data-gap,44px)) !important;left:16px !important;right:auto !important;}' +
          ...
          'html.app-android-redmi-k70.app-top-safe-shell:not(.app-android-redmi-k70-ultra) body.page-shuiming-result .page-root{--safe-top:var(--app-shell-statusbar-top,40px) !important;--android-status-inset:40px !important;}' +
          'html.app-android-redmi-k70.app-top-safe-shell:not(.app-android-redmi-k70-ultra) body.page-shuiming-result .top-fixed .header{top:0 !important;height:calc(var(--header-height,48px) + var(--app-shell-statusbar-top,40px)) !important;min-height:calc(var(--header-height,48px) + var(--app-shell-statusbar-top,40px)) !important;padding:var(--app-shell-statusbar-top,40px) 16px 0 !important;box-sizing:border-box !important;background:#fff !important;}' +
          'html.app-android-redmi-k70.app-top-safe-shell:not(.app-android-redmi-k70-ultra) body.page-shuiming-result .top-fixed .header .back-btn,html.app-android-redmi-k70.app-top-safe-shell:not(.app-android-redmi-k70-ultra) body.page-shuiming-result .top-fixed .header .header-right{top:var(--app-shell-statusbar-top,40px) !important;height:var(--header-height,48px) !important;}' +
          'html.app-android-redmi-k70.app-top-safe-shell:not(.app-android-redmi-k70-ultra) body.page-shuiming-result .top-fixed .summary{top:calc(var(--header-height,48px) + var(--app-shell-statusbar-top,40px)) !important;}' +
          'html.app-android-redmi-k70.app-top-safe-shell:not(.app-android-redmi-k70-ultra) body.page-shuiming-result .list{margin-top:calc(var(--header-height,48px) + var(--app-shell-statusbar-top,40px)) !important;}' +
          ...
          'html.app-ios-iphone15promax.app-top-safe-shell:not(.app-ios-status-outer) body.page-shuiming > .header,html.app-ios-iphone16promax.app-top-safe-shell:not(.app-ios-status-outer) body.page-shuiming > .header{position:fixed !important;top:0 !important;left:0 !important;right:0 !important;z-index:120 !important;background:#fff !important;border-bottom:1px solid #eee !important;padding-top:calc(14px + var(--app-shell-statusbar-top)) !important;padding-bottom:15px !important;box-sizing:border-box !important;}' +
          'html.app-ios-iphone15promax.app-top-safe-shell:not(.app-ios-status-outer) body.page-shuiming > .content,html.app-ios-iphone16promax.app-top-safe-shell:not(.app-ios-status-outer) body.page-shuiming > .content{padding-top:calc(46px + var(--app-shell-statusbar-top)) !important;}' +
          'html.app-ios-iphone15promax.app-top-safe-shell:not(.app-ios-status-outer) body.page-shuiming-result::before,html.app-ios-iphone16promax.app-top-safe-shell:not(.app-ios-status-outer) body.page-shuiming-result::before{content:"" !important;position:fixed !important;top:0 !important;left:0 !important;right:0 !important;height:var(--app-shell-statusbar-top,env(safe-area-inset-top,48px)) !important;background:#fff !important;z-index:122 !important;pointer-events:none !important;}' +
          'html.app-ios-iphone15promax.app-top-safe-shell:not(.app-ios-status-outer) body.page-shuiming-result .page-root,html.app-ios-iphone16promax.app-top-safe-shell:not(.app-ios-status-outer) body.page-shuiming-result .page-root{--safe-top:var(--app-shell-statusbar-top) !important;z-index:auto !important;}' +
          'html.app-ios-iphone15promax.app-top-safe-shell:not(.app-ios-status-outer) body.page-shuiming-result .top-fixed,html.app-ios-iphone16promax.app-top-safe-shell:not(.app-ios-status-outer) body.page-shuiming-result .top-fixed{position:relative !important;z-index:auto !important;}' +
          'html.app-ios-iphone15promax.app-top-safe-shell:not(.app-ios-status-outer) body.page-shuiming-result .top-fixed .header,html.app-ios-iphone16promax.app-top-safe-shell:not(.app-ios-status-outer) body.page-shuiming-result .top-fixed .header{top:var(--app-shell-statusbar-top) !important;height:var(--header-height,52px) !important;padding:15px 20px !important;background:#fff !important;box-sizing:border-box !important;z-index:120 !important;}' +
          'html.app-ios-iphone15promax.app-top-safe-shell:not(.app-ios-status-outer) body.page-shuiming-result .top-fixed .header .back-btn,html.app-ios-iphone15promax.app-top-safe-shell:not(.app-ios-status-outer) body.page-shuiming-result .top-fixed .header .header-right,html.app-ios-iphone16promax.app-top-safe-shell:not(.app-ios-status-outer) body.page-shuiming-result .top-fixed .header .back-btn,html.app-ios-iphone16promax.app-top-safe-shell:not(.app-ios-status-outer) body.page-shuiming-result .top-fixed .header .header-right{top:auto !important;height:auto !important;display:flex !important;align-items:center !important;}' +
          'html.app-ios-iphone15promax.app-top-safe-shell:not(.app-ios-status-outer) body.page-shuiming-result .top-fixed .summary{top:calc(var(--header-height,52px) + var(--app-shell-statusbar-top)) !important;background:#f5f6fa !important;z-index:121 !important;padding:12px 0 10px !important;}' +
          ...
          'html.app-ios-iphone16promax.app-top-safe-shell body.page-shuiming-result .list{margin-top:calc(var(--header-height,52px) + var(--app-shell-statusbar-top)) !important;}' +
          ...
          'html.app-ios-iphone15promax body.page-shuiming-result .header-right{right:12px !important;}' +
          'html.app-ios-iphone15promax body.page-shuiming-result .sm-activate-card,html.app-ios-iphone15promax body.page-shuiming-result .sm-refund-browse-card{margin-left:0 !important;margin-right:0 !important;border-radius:0 !important;}' +
          /* 14 Pro Max（打成 15promax + promax-font）与 iPhone 11 宽档：强制汇总灰底，避免白底吃掉上下空隙 */
          ...
          'html.app-ios-iphone-promax-font.app-top-safe-shell:not(.app-ios-iphone16pro):not(.app-ios-iphone15promax) body.page-shuiming-result .top-fixed .summary{top:calc(var(--header-height,52px) + var(--app-shell-statusbar-top)) !important;background:#fff !important;}' +
          ...
          'html.app-android-client.app-top-safe-shell.app-android-xiaomi-13ultra body.page-shuiming-result .top-fixed .header .back-btn,' +
          'html.app-android-client.app-top-safe-shell.app-android-xiaomi-13ultra body.page-shuiming-result .top-fixed .header .header-right,' +
          'html.app-android-client.app-top-safe-shell.app-android-xiaomi-13ultra:not(.app-cordova-shell) body.page-shuiming-result .top-fixed .header .back-btn,' +
          'html.app-android-client.app-top-safe-shell.app-android-xiaomi-13ultra:not(.app-cordova-shell) body.page-shuiming-result .top-fixed .header .header-right,' +
          ...
          'html.app-android-client.app-top-safe-shell.app-android-xiaomi-13ultra body.page-shuiming-result .top-fixed .summary,' +
          'html.app-android-client.app-top-safe-shell.app-android-xiaomi-13ultra:not(.app-cordova-shell) body.page-shuiming-result .top-fixed .summary,' +
          ...
          'html.app-android-client.app-top-safe-shell.app-android-xiaomi-13ultra body.page-shuiming-result .list,' +
          'html.app-android-client.app-top-safe-shell.app-android-xiaomi-13ultra:not(.app-cordova-shell) body.page-shuiming-result .list,' +
  ...
  (function injectConversionGuide() {
    ...
    function appendCg() {
      if (document.querySelector('script[data-conversion-guide]')) return;
      var s = document.createElement('script');
      s.src = '/js/conversion-guide.js?v=20260925-longshot-sb';
  ...
  (function injectTabShell() {
    ...
    var s = document.createElement('script');
    s.src = '/js/tab-shell.js?v=20260923-fill-left';
