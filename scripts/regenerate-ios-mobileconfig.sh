...
# 用法：HTTPS_APP_URL=https://139.199.191.204 ./scripts/regenerate-ios-mobileconfig.sh
set -euo pipefail
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
SRC="${ROOT}/个人.mobileconfig"
OUT_FRONTEND="${ROOT}/frontend/个人.mobileconfig"
OUT_ROOT="${ROOT}/个人.mobileconfig"
HTTPS_URL="${HTTPS_APP_URL:-https://139.199.191.204}"
TMP_PLIST="$(mktemp)"
...
python3 - "$TMP_PLIST" "$HTTPS_URL" "$OUT_FRONTEND" "$OUT_ROOT" <<'PY'
import plistlib, sys
