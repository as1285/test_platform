...
| API | `POST /api/auth.php`：`register` / `login` / `register_captcha`；`GET /api/auth.php` 状态类 |
...
| API | `POST /api/auth.php` `action=activate`；管理端发码 `POST /api/admin/issue-code` |
...
| API | `GET/POST /api/tax.php`：`list` 隐式 records、`save_record`、`batch_save_records`、`batch_replace_records`、软删/恢复等 |
...
| API | `/api/user.php`：`summary`、`save_profile`、`employers*`、`bank_cards*`、家人与专项附加 |
...
| API | `/api/public/install-packages`、`guest-session`、`landing-ab-config`、`conversion-config`、渠道归因；`user.php` / 公开埋点 `track_*` |
...
| API | `/api/message.php`、`/api/feedback.php`、`/api/chat.php` |
