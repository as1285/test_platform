...
const LIZHI_RENDER_SCRIPT = path.join(__dirname, '../../scripts/zaizhi_render_pdf.py');

function renderZaizhiPdfArtifacts(payload) {
  return new Promise(function (resolve, reject) {
    var tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'zaizhi-'));
...
function renderZaizhiPdfBuffer(payload) {
  return renderZaizhiPdfArtifacts(payload).then(function (art) {
    return art.pdf;
  });
}
...
function parseZaizhiStatsDays(raw) {
...
/** Avoid Illegal mix of collations between zaizhi_cert_generations and users. */
var USERNAME_JOIN =
...
/** C 端在职/工作证明：付费解锁 + 生成次数 */
async function handleAdminZaizhiCertStats(req, res) {
  try {
    var days = parseZaizhiStatsDays(req.query && req.query.days);
    ...
    try {
      const [unlockRows] = await conn.query(
        `SELECT COUNT(*) AS n FROM users WHERE zaizhi_cert_unlocked = 1`
      );
      const [paidSumRows] = await conn.query(
        `SELECT COUNT(*) AS orders,
         ...
         FROM payment_orders
         WHERE status = 'paid'
           AND (sku_id = ? OR grant_kind = 'zaizhi_cert')
           AND paid_at IS NOT NULL
           AND ${cnPaidDay}${sinceSql}`,
        [ZAIZHI_CERT_SKU_ID, days]
      );
      ...
      try {
        ...
        const [recentGenRows] = await conn.query(
          `SELECT g.id, g.username, g.demo, g.company_name, g.created_at,
                  u.real_name
           FROM zaizhi_cert_generations g
        ...
        const [usageRows] = await conn.query(
          `SELECT
             ...
             COALESCE(u.zaizhi_cert_unlocked, 0) AS unlocked,
           ...
           FROM (
             SELECT username FROM zaizhi_cert_generations
             WHERE ${cnCreatedDay}${sinceSql}
             UNION
           ...
           LIMIT 200`,
          [days, ZAIZHI_CERT_SKU_ID, days, days, ZAIZHI_CERT_SKU_ID, days]
        );
        usageUsers = (usageRows || []).map(mapUsageUserRow);
      } catch (genErr) {
        /* 表未迁移时仍返回付费统计 */
        console.error('[zaizhi-cert] stats generations', genErr);
      }
      ...
      const [recentPayRows] = await conn.query(
        `SELECT o.out_trade_no, o.username, o.amount, o.paid_at, o.status,
         ...
         WHERE o.status = 'paid'
           AND (o.sku_id = ? OR o.grant_kind = 'zaizhi_cert')
      ...
      var survey = await summarizeCertPageSurvey(conn, 'zaizhi', days);
      return res.json({
        code: 200,
        data: {
          period: {
    ...
    console.error('[zaizhi-cert] stats', e);
...
function getHandlers() {
  return {
    handleAdminZaizhiCertGenerate: handleAdminZaizhiCertGenerate,
    handleAdminZaizhiCertStats: handleAdminZaizhiCertStats
  };
}
